...
import filecmp
import logging
from struct5 import ObjectList, detect_type, ExecutableType, key_delimiter, object_delimiter
...
def diff_directory(dir1, dir2):
    def _recursive_dircmp(dcmp):
        _diffs = []
        for name in dcmp.left_only:
            _diffs.append([os.path.join(dcmp.left, name), Op.DELETED])
        for name in dcmp.right_only:
            _diffs.append([os.path.join(dcmp.right, name), Op.CREATED])
        for name in dcmp.diff_files:
            _diffs.append([os.path.join(dcmp.left, name), Op.UPDATED])

        # check recursively
        for sub_dcmp in dcmp.subdirs.values():
            sub_diffs = _recursive_dircmp(sub_dcmp)
            _diffs.extend(sub_diffs)
        return _diffs

    diff_files = _recursive_dircmp(filecmp.dircmp(dir1, dir2))
    return diff_files

def collection_file_path2object_key(path, root_dir):
    org_path = path
    short_path = org_path
    if not org_path.startswith(root_dir):
        raise ValueError("this path is not in the root dir: path \"\", root_dir \"\"".format(path, root_dir))
    short_path = org_path[len(root_dir):]
    if short_path.startswith("/"):
        short_path = short_path[1:]
    if short_path.startswith("ansible_collections/"):
        short_path = short_path[len("ansible_collections/"):]

    # change in tests is not treated as mutation
    if "/tests/" in short_path:
        return ""

    obj_key = ""
    parts = short_path.split("/")
    collection_name = ".".join(parts[:2])
    collection_path_prefix = "{}/{}/".format(parts[0], parts[1])
    if "plugins/modules/" in short_path:
        if short_path.endswith(".py") and not short_path.endswith("__init__.py"):
            module_name = short_path.split("/")[-1].replace(".py", "")
            obj_key = "module collection{}{}{}module{}{}.{}".format(key_delimiter, collection_name, object_delimiter, key_delimiter, collection_name, module_name)
    if "roles/" in short_path:
        if "/defaults/" in short_path or "/vars/" in short_path:
            role_name = short_path.split("roles/")[-1].split("/")[0]
            obj_key = "role collection{}{}{}role{}{}.{}".format(key_delimiter, collection_name, object_delimiter, key_delimiter, collection_name, role_name)
    if "tasks/" in short_path:
        if short_path.endswith(".yml") or short_path.endswith(".yaml"):
            taskfile_path_in_collection = short_path.replace(collection_path_prefix, "")
            obj_key = "taskfile collection{}{}{}taskfile{}{}".format(key_delimiter, collection_name, object_delimiter, key_delimiter, taskfile_path_in_collection)
    return obj_key

def convert(tree, node_objects, mutated_keys):
    node_dict = {}
    ...
    def getSubTree(node, root=False):
        ...
        if node_type == "module":
            ...
            if "module" in children_per_type:
                ...
                if len(tmp_children) > 0 and tmp_children[0].get("mutated", False):
                    obj["module_mutated"] = True
        # end
        return obj
...
def main():
    ...
    parser.add_argument('--current', default="", help='path to a directory of dependencies for the tree.json')
    parser.add_argument('--target', default="", help='path to a directory of dependencies to be compared')
    ...
    if args.current == "" or args.target == "":
        logging.error("\"--current\" and \"--target\" are required")
        sys.exit(1)

    diff_files = diff_directory(args.current, args.target)

    mutated_keys = {}
    for diff in diff_files:
        fpath = diff[0]
        op = diff[1]
        dir = args.target if op == Op.CREATED else args.current
        obj_key = collection_file_path2object_key(fpath, dir)
        mutated_keys[obj_key] = op

    trees = load_tree_json(args.tree_file)
    objects = load_node_objects(args.node_file, args.root_dir, args.ext_dir)

    for tree in trees:
        t_obj, context = convert(tree, objects, mutated_keys)
