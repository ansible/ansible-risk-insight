...
# inbound? read remote file
class SlurpAnnotator(ModuleAnnotator):
    fqcn: str = "ansible.builtin.slurp"
    enabled: bool = True

    def run(self, task: TaskCall) -> List[Annotation]:
        src = task.args.get("src")

        annotation = RiskAnnotation.init(risk_type=DefaultRiskType.INBOUND, detail=InboundTransferDetail(_src_arg=src))
